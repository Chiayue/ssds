...
LinkLuaModifier( "modifier_archon_passive_fire_particles", "ability/archon_passive_fire.lua",LUA_MODIFIER_MOTION_NONE )
-------------------------------------------------
--Abilities
...
function modifier_archon_passive_fire:OnAttackLanded( params )
	...
	if hAttacker == self:GetParent() and hAttacker:HasAbility("archon_passive_fire") then
		...
		SendParticlesToClient("particles/heroes/mouko/ability_mokou_01_boom.vpcf",hTarget)
		-- 范围
...
function modifier_archon_passive_fire:OnIntervalThink(params)
	if IsServer() then
		local nAllRange = self:GetCaster():GetBaseAttackRange() + GetUnitRange(self:GetCaster())
		...
		for _,enemy in pairs(enemies) do
			if enemy ~= nil  then
				...
				ApplyDamage( damage )
				SendParticlesToClient(impact_effect,enemy,PATTACH_OVERHEAD_FOLLOW)
			end
		end
		enemies = nil
	end
...
--------------
modifier_archon_passive_fire_particles = {}
function modifier_archon_passive_fire_particles:GetAttributes() return  MODIFIER_ATTRIBUTE_MULTIPLE end
function modifier_archon_passive_fire_particles:IsDebuff() return false end
function modifier_archon_passive_fire_particles:IsHidden() return true end
function modifier_archon_passive_fire_particles:OnCreated()
