...
LinkLuaModifier("modifier_wearable_hider_while_model_changes", "heroes/wearable_manager.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_wearable_visuals_illusion", "heroes/wearable_manager.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_wearable_hider_while_model_changes:OnDeath(keys)
	...
	if not hParent.tWearables then return end
...
function modifier_wearable_hider_while_model_changes:OnModelChanged(keys)
	...
	local hParent = self:GetParent()
	-- 风行至宝
	if self:GetParent():GetModelName() == "models/items/windrunner/windrunner_arcana/wr_arcana_base.vmdl" then
		self:GetParent():SetModel("models/heroes/windrunner/windrunner.vmdl")
	end
	if self.sOriginalModel == self:GetParent():GetModelName() then	
		...
		--print("false")
		self:GetParent():SetModel(self.sOriginalModel)
		for k, v in pairs(hParent.tWearables) do
...
function WearableManager:AddNewWearable(hUnit, tInput, tItemGame)		
	...
	local tParticles = {}
	local GameItems
	local hOriginalWearable = tInput.hOriginalWearable
	if tItemGame then
		GameItems = tItemGame
	else
		GameItems = _G.GameItems
	end
	
	if sModel then
		if hOriginalWearable then
			hWearable = hOriginalWearable
			...
			for k1, v1 in pairs(tParticles) do
				if v1.system == v0.system then
					...
					if v0.control_points then 
						...
						for k2, v2 in pairs(v1.control_points) do
							if type(v2.attach_type) == 'string' then
...
function WearableManager:RemoveWearableParticleByIndex(hUnit, sID)
	if not hUnit.tWearables then return end
	for k, v in pairs(hUnit.tWearables) do
		if v.ID == sID then
			for k1, v1 in pairs(v.particle_systems) do 
...
function WearableManager:RemoveOriginalWearables(hUnit)
	local sOriginalModel = hUnit:GetModelName()
	if sOriginalModel == "models/items/windrunner/windrunner_arcana/wr_arcana_base.vmdl" then
		hUnit:SetModel("models/heroes/windrunner/windrunner.vmdl")
	end
	for i, v in pairs(hUnit:GetChildren()) do
		if v ~= nil then
			if v:GetClassname() == 'dota_item_wearable' then
				local sModelName = v:GetModelName() 
				--print(sModelName)
